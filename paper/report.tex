...
The interval partitioning problem asks ``Given a set of tuples $\{(s_1, f_1), (s_2, f_2), \ldots, (s_n, f_n)\}$, what is the minimum number of containers needed for

% BEGIN COMMENT OF OLD MATERIAL
\comment{After considering chromatic numbers on graphs of jobs and integer programming on times slots, we settled on describing the problem using the bin packing problem.
...
For our problem, this approximation is plenty accurate as well as when we consider the fact that the truly ``optimal'' algorithm could be suboptimal anyways when taking into account our other constraints.} % END COMMENT OF OLD MATERIAL
